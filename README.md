# Bible Web Services (BWS) - API Bible

Le projet API Bible (Bws.Bible.Api) est un **web service** REST qui permet d'explorer la Bible, mais aussi d'effectuer des recherches et de comparer différentes traductions de la Bible.


## Pour commencer

//TODO

### Pré-requis

//TODO

### Installation

//TODO

## Démarrage

//TODO


## Fabriqué avec

- [Visual Studio 2019](https://visualstudio.microsoft.com/fr/vs/) - EDI C# dotnet core
- [GIT](https://git-scm.com/) - Système de contrôle de version distribué
- [Azure DevOps](https://azure.microsoft.com/fr-fr/services/devops/) - Gestion CI/CD, industrialisation logicielle


## Contributing

Si vous souhaitez contribuer, lisez le fichier [CONTRIBUTING.md](./CONTRIBUTING.md) pour savoir comment le faire.


## Versions

- **Dernière version stable :** 1.0.0
- **Dernière version :** 1.0.0
- Liste des versions : 


## Auteurs
Pour en savoir plus sur les auteurs du projet, c'est ici !

* **Samuel MEYER** - conception et développement initial du projet - [www.samuel-meyer.fr](https://www.samuel-meyer.fr)


## License

Ce projet est distribué sous deux régimes de droits distincts :
- **Code source :** ``MIT License`` -
- **Données et contenus :** ``Licence Creative Commons BY-NC-ND`` - 
Voir le fichier [LICENSE.md](LICENSE.md) pour plus d'informations.
